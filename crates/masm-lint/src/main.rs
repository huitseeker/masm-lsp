...
fn root_cause_group_to_lint(group: &AdviceRootCauseGroup) -> LintDiagnostic {
    ...
    LintDiagnostic {
        ...
        span: group.origin,
        note: grouped_procedure_note(&procedures),
        related,
    }
}

fn grouped_procedure_note(procedures: &[String]) -> String {
    match procedures {
...
#[cfg(test)]
mod tests {
    use masm_analysis::{AdviceDiagnostic, AdviceRootCauseGroup, AdviceSinkKind};
    ...
    #[test]
    fn grouped_procedure_note_summarizes_multiple_procedures() {
        let procedures = vec![
            "alpha::sink".to_string(),
            "beta::sink".to_string(),
            "gamma::sink".to_string(),
        ];

        assert_eq!(
            grouped_procedure_note(&procedures),
            "root cause fan-out reaches procedures `alpha::sink`, `beta::sink`, and 1 more"
        );
    }

    #[test]
    fn root_cause_group_to_lint_uses_origin_as_primary_span() {
        let origin = SourceSpan::new(SourceId::new_unchecked(1), 1u32..4u32);
        let sink = SourceSpan::new(SourceId::new_unchecked(1), 8u32..12u32);
        let group = AdviceRootCauseGroup {
            origin,
            diagnostics: vec![AdviceDiagnostic::new(
                SymbolPath::new("demo::sink"),
                sink,
        ...
        let lint = root_cause_group_to_lint(&group);

        assert_eq!(lint.span, origin);
        assert_eq!(lint.related.len(), 1);
        assert_eq!(lint.related[0].span, sink);
    }
}
