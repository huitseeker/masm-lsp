...
mod tests {
    ...
    #[test]
    fn join_is_associative_and_idempotent() {
        let values = [U32Validity::Unknown, U32Validity::ProvenU32];
        for lhs in values {
            assert_eq!(lhs.join(lhs), lhs);
            for rhs in values {
                assert_eq!(lhs.join(rhs), rhs.join(lhs));
                for other in values {
                    assert_eq!(lhs.join(rhs).join(other), lhs.join(rhs.join(other)));
                }
            }
        }
    }
