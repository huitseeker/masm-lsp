//! One-off stdlib evaluation harness for the Phase 2 `u32` demo.
...
/// Return whether one advice diagnostic belongs to the `u32`-focused Phase 2 demo.
fn is_u32_demo_diagnostic(diag: &AdviceDiagnostic) -> bool {
    diag.is_u32_demo_sink()
}
...
#[test]
#[ignore = "one-off Phase 2 stdlib evaluation; set MASM_PHASE2_STDLIB_ROOT and run with --ignored --nocapture"]
fn phase2_stdlib_u32_noise_report() {
    ...
    let u32_diagnostics = snapshot
        ...
        .filter(|diag| is_u32_demo_diagnostic(diag))
        .collect::<Vec<_>>();
